program Obligatorio3
  ...
  do n = 0, 1000
     ...
     ! Escritura de posiciones y partes real e imaginaria de φ en 'xreim.dat'
     do j = 0, S
     ...
     ! Cálculo de la energía <H> = -1/2 ∫ φ* φ'' dx
     ep = (0.d0, 0.d0)
     do j = 1, S-1
         ep = ep + conjg(phi(j))*(phi(j+1)+phi(j-1)-2.d0*phi(j))*(-1.d0)/(2.d0*dx**2)*dx
     end do
     write(13,*) t, real(ep)
